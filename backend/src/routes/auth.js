...
router.post('/register', async (req, res) => {
  ...
  if (process.env.SMTP_USER) {
    ...
    transporter.sendMail({
      ...
      text: `New user: ${firstName} ${lastName}, Phone: ${phone}, Omang: ${omang}, Referral: ${referredBy || 'Direct'}`
    });
  }

  res.json({ message: 'Registered!', referralLink: `${req.headers.origin}/r/${referralCode}` });
});
...
router.post('/login', async (req, res) => {
  ...
  res.json({ message: 'Logged in', user: { name: `${user.firstName} ${user.lastName}`, phone } });
});
